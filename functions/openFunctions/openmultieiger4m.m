function out = openmultieiger4m(varargin)
% ---
...
data1  = 0                                                                 ; %#ok<*NASGU> % switch for first data file
if numel(name) > 7 && strcmpi(name(end-6:end),'_master') == 1 %#ok<*COLND>
    master     = 1                                                         ;
    masterfile = file                                                      ;
...
if data == 1
    dataInfo = h5info(file, '/entry/data/data/')                           ; % load the data info
    if EndIndex == inf
    ...
    if dataInfo.Dataspace.Size(3) >= StartIndex && dataInfo.Dataspace.Size(3) >= EndIndex
        for j = 1 : EndIndex - StartIndex + 1
            img = h5read(file,'/entry/data/data/',[1 1 StartIndex+j-1],[inf inf 1])     ;
            if j == 1
                try
...
elseif master == 1
    if data1 == 1                                                            % (at least the first data file exists)
        ...
        && dataInfo.Dataspace.Size(3) >= EndIndex                          
            for j = 1 : EndIndex - StartIndex + 1
                img = h5read(datafile1,'/entry/data/data/',[1 1 StartIndex+j-1],[inf inf 1]) ;
                if j == 1
                    try
                        out.imm = zeros(size(img,2),size(img,1),EndIndex-StartIndex+1,'single')  ;
                    catch EM
            ...
            if goal1 == goal2
                ...
                && dataInfoGoal.Dataspace.Size(3) >= newEndIndex            % the goal data file contains the 'wanted' image
                    for j = 1 : newEndIndex - newStartIndex + 1
                        img = h5read(datafilegoal,'/entry/data/data/',[1 1 newStartIndex+j-1],[inf inf 1]) ;
                        if j == 1
                            try
            ...
            elseif (goal2 - goal1) == 1
                ...
                && dataInfoGoal2.Dataspace.Size(3) >= newEndIndex            % the goal data sets contain the 'wanted' images
                    for k = newStartIndex : dataInfoGoal1.Dataspace.Size(3)
                        img = h5read(datafilegoal1,'/entry/data/data/',[1 1 k],[inf inf 1]) ;
                        if k == newStartIndex
                            try
                                out.imm = zeros(size(img,2),size(img,1),EndIndex-StartIndex+1,'single')  ;
                            catch EM
                    ...
                    for k = 1 : newEndIndex
                        img = h5read(datafilegoal2,'/entry/data/data/',[1 1 k],[inf inf 1]) ;
                        j = j + 1                                          ;
                        out.imm(:,:,j) = single(img.')                     ;
                    end
                else
                    disp(sprintf('Error: Images could not be retrieved from data sets!')) ;
                    return
                end
                ...
                && dataInfoGoal2.Dataspace.Size(3) >= newEndIndex            % the goal data sets contain the 'wanted' images
                    ...
                    for n = 1 : goal2 - goal1 - 1
                        for k = 1 : dataInfoGoaln(n,3)
                            img = h5read(datafilegoaln{n},'/entry/data/data/',[1 1 k],[inf inf 1]) ;
                            j = j + 1                                      ;
                            out.imm(:,:,j) = single(img.')                 ;
                        end
                    end
