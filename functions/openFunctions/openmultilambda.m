function out = openmultilambda(varargin)
% ---
% --- OPENMULTILAMBDA Open LAMBDA hdf5 (*.h5) data files
% ---
% --- USAGE:
% --- OUT = OPENMULTILAMBDA(FILENAME)        --> direct call to 
% --- open all images of a HDF5 or NEXUS file
% --- OUT = OPENMULTILAMBDA(FILENAME,STARTINDEX,ENDINDEX)--> call to 
% --- open a block of images. 
% --- Here filename stands for the first file of the batch and the 
% --- corresponding filename is created by cutting the filename after
% --- the last underscore and adding the index to the remaining namestring.
% ---
% --- Input Argument:
...
% ---
% --- M. Sprung
% --- $Revision: 1.0 $Date: 2015/05/28 $ function to open LAMBDA hdf5 files
% --- $Revision: 1.0 $Date: 2015/06/18 $ function to open LAMBDA NXS files
% ---
...
% =========================================================================
hasMovingHotPixels = 1                                                     ; % Switch to indicate if the data contains 'moving hot pixels'
...
clear MyStartTime MyEndTime DistortionCorrection CompareDateStr CompareDate
...
if strcmpi(ext,'.h5') == 1 || strcmpi(ext,'.nxs') == 1
    ...
    if (sum(strcmp('count_time', FirstDatasets)) == 1)
        count_time = h5read(file, Data_prefix2) / 1000 ;
    end
end

% --- Load in additional information
if (exist('SecondDatasets', 'var') == 1)
    try
        Data_prefix11       = [child5_full_name, '/delay_time/']           ;
        Delay_time          = h5read(file, Data_prefix11)                  ;
    catch
    end
    try
        % Data_prefix12       = [child5_full_name, '/frame_numbers/']        ;
        Data_prefix12       = [child5_full_name, '/frame_no/']             ;
        Frame_Number        = h5read(file, Data_prefix12)                  ;
    catch
    end
    try
        Data_prefix13       = [child5_full_name, '/total_loss_frames/']    ;
        Total_Loss          = h5read(file, Data_prefix13)                  ;
    catch
    end
    try
        Data_prefix14       = [child5_full_name, '/shutter_time/']         ;    
...
% --- Check input variables for consistency
if StartIndex >= Datasize(3)
    StartIndex = Datasize(3)                                               ;
end
if isfinite(EndIndex) && EndIndex>= Datasize(3)
    EndIndex = inf                                                         ;
end
...
% =========================================================================
imm            = h5read(file, Data_prefix1, [1 1 StartIndex], [inf inf EndIndex-StartIndex+1]) ;
imm            = single(imm)                                               ;
...
if hasMovingHotPixels == 1
    ...
    elseif numel(size(imm)) == 3
        for j = 1 : size(imm,3)
            imm(746:775,1:255,j) = lambda_correcthotpixel(imm(746:775,1:255,j))   ;           
        end
    end
end
